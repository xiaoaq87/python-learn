...
# 4、DeepSeek 只⽀持 Chat Completions API，需要显式指定
set_default_openai_api('chat_completions')

# 5、创建兼容OpenAI格式的deepseek客户端
deepseek_clint = AsyncOpenAI(
    api_key=deepseek_api_key,
    base_url=deepseek_base_url,
)

# 6、把自定义客户端设为sdk默认使用的客户端,use_for_tracing=False阻止了追踪功能使用该客户端的API密钥但没有关闭追踪。
set_default_openai_client(deepseek_clint, use_for_tracing=False)
